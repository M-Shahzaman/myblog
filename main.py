...
class User(db.Model, UserMixin):
    ...
    # List of blogs attached to a user
...
def get_all_posts():
    ...
    return render_template("index.html", all_posts=posts, loged_in=True)
...
def register():
    ...
    if form.validate_on_submit():
        visitor = User.query.filter_by(email=form.email.data).first()
        if visitor:
            flash('This email is already registered, Login instead!')
            return redirect(url_for('login'))
        has_paswd = generate_password_hash(password=form.password.data, method='pbkdf2:sha256', salt_length=8)
        new_user = User(
            name=form.name.data,
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash('This email is no registered, Quickly register!')
